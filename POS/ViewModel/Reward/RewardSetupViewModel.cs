    ...
    class RewardSetupViewModel : ViewModelBase, IDataErrorInfo
    {
        ...
        private void Update()
        {
            try
            {
                ...
                && (checkReward != null && checkReward.Count() > 0))
                {
                    ...
                    MessageBoxResultCustom result = MsgControl.ShowWarning(message, "Notification", MessageBoxButtonCustom.YesNoCancel, MessageBoxImage.Question);
                ...
                if (this.NumbersOfRewardRelation > 0)
                    MsgControl.ShowInfomation(String.Format(Application.Current.FindResource("RW_Message_Issued") as string, this.NumbersOfRewardRelation), Language.Information, MessageBoxButtonCustom.OK);
            }
            catch (Exception ex)
            {
        ...
        private bool IsEditData()
        {
            ...
            if (this.RewardManagerModel != null)
            {
                ...
                {
                    MessageBoxResultCustom msgResult = MsgControl.ShowWarning(Language.Text13, Language.Warning, MessageBoxButtonCustom.YesNo);
