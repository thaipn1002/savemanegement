            ...
            foreach (base_GuestRewardModel guestRewardModel in saleOrderModel.GuestModel.GuestRewardCollection.Where(x => x.ActivedDate.Value.Date <= DateTime.Today.Date && (!x.ExpireDate.HasValue || (x.ExpireDate.HasValue && saleOrderModel.OrderDate.Value.Date <= x.ExpireDate.Value))))
            {
                ...
                if (guestReward != null)
                {
                    ...
                    if (guestReward.RewardSetupUnit.Equals((short)RewardType.Money))
                    {
                        ...
                        if (guestReward.RewardSetupAmount > saleOrderModel.Total)
                        {
                            decimal requiredAmount = guestReward.RewardSetupAmount - subTotal;
                        ...
                        {
                            guestReward.TotalAfterReward = subTotal - guestReward.RewardSetupAmount;
                    ...
                    else
                    {
                        ...
                        guestReward.RewardName = string.Format("{0}% off; expires {1}", string.Format(Define.ConverterCulture, "{0:N0}", guestReward.RewardSetupAmount), msgExpiredDate);
                        guestReward.TotalAfterReward = saleOrderModel.Total - (subTotal * guestReward.RewardSetupAmount / 100);
