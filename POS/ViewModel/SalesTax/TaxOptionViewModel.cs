...
namespace CPC.POS.ViewModel
{
    class TaxOptionViewModel : ViewModelBase
    {
        ...
        {
            //CurrencyFormat = Define.CurrencyFormat;
            //ConverterCulture = Define.ConverterCulture;
            InitialCommand();
        }
        #endregion
        ...
        private bool _allowEditSaleTax;
        /// <summary>
        ...
        public CultureInfo ConverterCulture
        {
            get
            {
                if (_converterCulture == null)
                    this._converterCulture = Define.ConverterCulture;
                return _converterCulture;
            }
            set
            {
                if (_converterCulture != value)
                {
                    _converterCulture = value;
                    OnPropertyChanged(() => ConverterCulture);
                }
            }
        }
        ...
        public string CurrencyFormat
        {
            get
            {
                if (string.IsNullOrEmpty(this._currencyFormat))
                    this._currencyFormat = Define.CurrencyFormat;
        ...
        public string Price
        {
            get
            {
                if (string.IsNullOrWhiteSpace(this._price))
                    this._price = "10";
                return _price;
            }
            set
        ...
        private bool OnAddTaxRateOptionCommandCanExecute(object param)
        {
            if (SaleTaxLocationOptionCollection == null)
                return false;
            return !SaleTaxLocationOptionCollection.Any(x => x.IsError) && SaleTaxLocationOptionCollection.Count() < Define.MultiTaxMaxLength;//All Item in collection Not error
        }
