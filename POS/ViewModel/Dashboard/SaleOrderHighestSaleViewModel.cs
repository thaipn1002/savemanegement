...
{
    public class SaleOrderHighestSaleViewModel : ViewModelBase, IDashboardItemFunction
    {
        #region Fields
        ...
        public SaleOrderHighestSaleViewModel(Grid grid, XElement configuration)
        {
            _gridView = grid;
            ...
            _chartSeries.DisplayMember = "SONumber";
        ...
        private void GetSaleOrders()
        {
            try
            {
                ...
                string query = string.Format(
                    ...
                    "Where \"StoreCode\" = {0} And \"OrderStatus\" <> {1} And \"OrderStatus\" <> {2} And \"SubTotal\" > 0 " +
                ...
                {
                    SONumber = x.Field<string>("SONumber"),
                ...
                if ((ChartType)_chartTypeID == ChartType.PieChart || (ChartType)_chartTypeID == ChartType.DoughnutChart)
                {
                    ...
                    _chartBase.ChartSubTitle = "SubTotal";
                }
                _chartBase.ChartTitle = string.Format("Top {0} Sale Order With Highest Sale", _total);
                _chartSeries.SeriesTitle = "SubTotal";
                _chartSeries.ValueMember = "SubTotal";
            }
