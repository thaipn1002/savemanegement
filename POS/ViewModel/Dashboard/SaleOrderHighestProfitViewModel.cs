...
{
    public class SaleOrderHighestProfitViewModel : ViewModelBase, IDashboardItemFunction
    {
        #region Fields
        ...
        public SaleOrderHighestProfitViewModel(Grid grid, XElement configuration)
        {
            _gridView = grid;
        ...
        private void GetSaleOrders()
        {
            try
            {
                ...
                string query = string.Format(
                    "Select * From v_sale_profit_summary " +
                    "Where \"StoreCode\" = {0} And \"OrderStatus\" <> {1} And \"OrderStatus\" <> {2} And \"Gross Profit\" > 0 " +
                ...
                _chartBase.ChartTitle = string.Format("Top {0} Sale Order With Highest Profit", _total);
