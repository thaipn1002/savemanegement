...
namespace CPC.POS.ViewModel
{
    class ChangeWorkScheduleViewModel : ViewModelBase, IDataErrorInfo
    {
        ...
        private void OnOkCommandExecute()
        {
            ...
            foreach (base_GuestModel employeeItem in RightEmployeeCollection)
            {
                employeeItem.NextEmployeeSchedule.WorkScheduleId = ChangeToWorkSchedule.Id;

                if (employeeItem.EmployeeScheduleModel == null || (employeeItem.EmployeeScheduleModel.StartDate < DateTimeExt.Today &&
                    employeeItem.EmployeeScheduleModel.Status != (int)ScheduleStatuses.Pending))
                {
                    // Map data from model to entity
                    employeeItem.NextEmployeeSchedule.ToEntity();

                    // Add new employee schedule to database
                    _employeeScheduleRepository.Add(employeeItem.NextEmployeeSchedule.base_GuestSchedule);
                }
                else
                {
                    // Update current employee schedule
                    employeeItem.EmployeeScheduleModel.WorkScheduleId = employeeItem.NextEmployeeSchedule.WorkScheduleId;
                    employeeItem.EmployeeScheduleModel.StartDate = employeeItem.NextEmployeeSchedule.StartDate;
                    employeeItem.EmployeeScheduleModel.AssignDate = employeeItem.NextEmployeeSchedule.AssignDate;
                ...
                if (employeeItem.NextEmployeeSchedule.IsNew)
                {
                ...
                employeeItem.NextEmployeeSchedule.EndUpdate();
            }
            ...
            if (_leftResultFilterList != null)
            {
                foreach (base_GuestModel employeeModel in _leftResultFilterList.Cast<base_GuestModel>().Where(x => x.IsChecked).ToList())
                {
                    ...
                    if (employeeModel.NextEmployeeSchedule == null)
                        employeeModel.NextEmployeeSchedule = new base_GuestScheduleModel();
                    employeeModel.NextEmployeeSchedule.GuestId = employeeModel.Id;
                    employeeModel.NextEmployeeSchedule.StartDate = StartDate.Value;
                    employeeModel.NextEmployeeSchedule.AssignDate = DateTimeExt.Now;
                    employeeModel.NextEmployeeSchedule.Status = (int)EmployeeScheduleStatuses.Pending;
        ...
        private void LoadLeftEmployeeCollection(tims_WorkScheduleModel workScheduleModel)
        {
            if (LeftEmployeeCollection.Count == 0)
            {
                ...
                foreach (base_GuestSchedule employeeSchedule in employeeSchedules)
                {
                    ...
                    employeeModel.EmployeeScheduleModel = new base_GuestScheduleModel(employeeSchedule.base_Guest.base_GuestSchedule.
                        Where(x => x.WorkScheduleId == workScheduleID).OrderBy(x => x.StartDate).LastOrDefault());
