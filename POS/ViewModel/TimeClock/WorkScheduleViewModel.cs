    ...
    class WorkScheduleViewModel : ViewModelBase
    {
        ...
        private void OnPopupChangeWorkScheduleCommandExecute(object param)
        {
            ...
            if (result.HasValue && result.Value)
            {
                ...
                foreach (tims_WorkScheduleModel workScheduleItem in WorkScheduleCollection.Where(x => x.Id.Equals(currentWorkScheduleID) || x.Id.Equals(changeToWorkScheduleID)))
                {
                    workScheduleItem.NumberOfEmployees = workScheduleItem.tims_WorkSchedule.base_GuestSchedule.
                        Count(x => x.base_Guest.IsActived && x.Status > (int)EmployeeScheduleStatuses.Inactive);
                }
            }
        }
        ...
        {
            workScheduleModel.NumberOfEmployees = workScheduleModel.tims_WorkSchedule.base_GuestSchedule.
                Count(x => x.base_Guest.IsActived && x.Status > (int)EmployeeScheduleStatuses.Inactive);
        }
        ...
        private void SaveUpdate(tims_WorkScheduleModel workScheduleModel)
        {
            ...
            // Update work week and employee schedule
