    ...
    class PermissionViewModel  : ViewModelBase
    {
        ...
        public void SaveCommandExecute()
        {
            ...
            {                                
                if (!isClone)
                {
                    // Update User Menu permission and User Group permission
        ...
        {
            return (UserModel != null && !UserModel.IsDirty && UserModel.Errors.Count==0 && UserModel.UserName != Common.LOGIN_NAME && UserModelCollection.Count > 1);
        }
        #endregion
        ...
        private bool CheckDuplicateLoginName()
        {
            ...
            {
                var resuilt = userRepo.Get(x => x.LoginName.ToLower().Equals(UserModel.LoginName.ToLower()));
                return (resuilt != null);
            }
            return false;
        }
        ...
        private void UpdateNewUserRight()
        {
            ...
            {
                if (uMPermissions[i].Code == crrListUMPermission[i].Code)
                {
                    uMPermissions[i].Right = crrListUMPermission[i].Right;
            ...
            for (i = 0; i < rowCount; i++)
            {
                if (uGPermissions[i].Code == crrListUGPermission[i].Code)
                {
                    uGPermissions[i].Right = crrListUGPermission[i].Right;
            ...
            for (i = 0; i < rowCount; i++)
            {
                if (uRPermissions[i].Code == crrListURPermission[i].Code)
                {
                    uRPermissions[i].IsPrint = crrListURPermission[i].IsPrint;
                    uRPermissions[i].IsView = crrListURPermission[i].IsView;
        ...
        {
            // Get all Permission
            var lstPermission = new ObservableCollection<rpt_PermissionModel>(
                    permissionRepo.GetAll()
                    .Select(u => new rpt_PermissionModel(u))
                    .Where(w => w.UserResource == UserModel.Resource.ToString())
                    .OrderBy(o => o.Type)
        ...
        private void SetURPermission()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                if (ReportCollection[i].Code == URPermissionModelCollection[i].Code)
                {
                    URPermissionModelCollection[i].Name = ReportCollection[i].Name;
        ...
        private void SetUMPermission()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                if (MenuCollection[i].Code == UMPermissionModelCollection[i].Code)
                {
        ...
        {           
            ...
            for (int i = 0; i < count; i++)
            {
                if (GroupCollection[i].Code == UGPermissionModelCollection[i].Code)
                {
        ...
        private void CheckAllView_Print()
        {
            IsCheckView = true;
            IsCheckPrint = true;
