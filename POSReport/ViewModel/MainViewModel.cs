    ...
    class MainViewModel : ViewModelBase
    {
        ...
        private void GetGiftCertificateList()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                byte[] isSold = null;
                if (da.Rows[i][9] != DBNull.Value)
                {
                    isSold = bool.Parse(da.Rows[i][9].ToString()) ? trueImg : falseImg;
                }
        ...
        private void ExportFile(CrystalDecisions.CrystalReports.Engine.ReportDocument obj)
        {
            ...
            string exportFile = folderName + "\\" + DateTime.Now.ToString("yyyy-MM-dd hhmmss") + ".pdf";
            ...
            if (!string.IsNullOrWhiteSpace(ReportModel.CCReport))
            {
                ...
                Common.EMAIL_ACCOUNT = da.Rows[0][2].ToString();
                Common.EMAIL_PWD = da.Rows[0][3].ToString();
                da.Clear();
                string reportName = ReportModel.Name;
                string attachFile = exportFile;
                string errorList = Common.SendEmail(ReportModel.CCReport, reportName, attachFile);
        ...
        private void ConvertImageToByteArray()
        {
            try
            {
                ...
                path = "CPC.POSReport.Image.TrueFalseImgs.False.jpg";
                stream = System.Reflection.Assembly.GetEntryAssembly().GetManifestResourceStream(path);
                using (System.IO.MemoryStream ms = new System.IO.MemoryStream())
                {                   
                    stream.CopyTo(ms);
                    falseImg = ms.ToArray();
        ...
        private void SetMenuPermission()
        {
            ...
            for (int i = 0; i < rowCount; i++)
            {
                if (int.Parse(da.Rows[i][0].ToString()) != 2)
                {
                    continue;
                }
                switch (da.Rows[i][1].ToString())
                {
                    ...
                    case "M06":
                        ...
                        break;
                    case "M07":
                        Common.VIEW_HISTORY_PRINTED = (bool)da.Rows[i][4];
                        break;
                    case "M08":
                        Common.ADD_USER = (bool)da.Rows[i][4];
                        break;
                    case "M09":
                        Common.EDIT_USER = (bool)da.Rows[i][4];
                        break;
                    case "M10":
                        Common.DELETE_USER = (bool)da.Rows[i][4];
                        break;
                    case "M11":
