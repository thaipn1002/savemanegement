...
namespace CPC.POS.ViewModel
{
    class ReportViewModel : ModelBase
    {
            ...
            try
            {
                ...
                if (da.Rows.Count > 0)
                {
                    ds.Tables["CompanyInfo"].Rows.Add(da.Rows[0][0], da.Rows[0][1], da.Rows[0][2],
                        da.Rows[0][3], da.Rows[0][4], da.Rows[0][5], da.Rows[0][6], da.Rows[0][7]
                        );
                    // Set currency symbol
                ...
                else
                {
                    ...
                    if (isSendEmail)
                    {
                        ...
                        // Get customer name
                        string customerName = saleOrder.GuestModel.LastName + " " + saleOrder.GuestModel.FirstName + " " + saleOrder.GuestModel.MiddleName;
            ...
            if (da.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < da.Rows.Count; i++)
                {
                    ...
                    ds.Tables["PickPack"].Rows.Add(
                            da.Rows[i][0], da.Rows[i][1], da.Rows[i][2], da.Rows[i][3],
                            da.Rows[i][4], isReturned, da.Rows[i][6], da.Rows[i][7]
                        );
                }
                da.Clear();
            }
            ...
            try
            {
                ...
                if (count > 0)
                {
                    purchaseOrderDataSet.Tables["CompanyInfo"].Rows.Add(
                        ...
                        da.Rows[0][4], da.Rows[0][5], da.Rows[0][6], da.Rows[0][8]);
                }                
                // Get purchase order 
                ...
                {                                        
                    if (type == "PReturn")
                    {
                        ...
                        purchaseOrderReport.ReportDefinition.ReportObjects["txtBalance1"].Left = 9320;
        ...
        public static string SendEmail(string mailTo, string subject, string attachmentFile, string customerName)
        {
            ...
            //    mailMessage.From = new MailAddress(EMAIL_ACCOUNT, "SmartPOS");                
            ...
            try
            {
                ...
                if (obj != DBNull.Value)
                {
                    ...
                    switch (param)
                    {
                        ...
                        case "PaymentMethods":
                            name = Common.PaymentMethods.Single(x => x.Value == key).Text;
                            break;
                        // Get status by key
                        case "StatusBasic":
                            name = Common.StatusBasic.Single(x => x.Value == key).Text;
                            break;
                        // Get State by key
                        case "states":
                            name = Common.States.Single(x => x.Value == key).Text;
                            break;
                        // Get country by key
                        case "country":
                            name = Common.Countries.Single(x => x.Value == key).Text;
                            break;
                        // Get Title by key
                        case "Title":
                            name = Common.Title.Single(x => x.Value == key).Text;
                            break;
                        // Get Marital status by key
                        case "MaritalStatus":
                            name = Common.MaritalStatus.Single(x => x.Value == key).Text;
                            break;
                    }
                }
                return name;
        ...
        {            
            ...
            ds.Tables["SOHeader"].Rows.Add(
                dtConfig.Rows[0][0], dtConfig.Rows[0][1], dtConfig.Rows[0][2], dtConfig.Rows[0][3],
                dtConfig.Rows[0][4], dtConfig.Rows[0][5], dtConfig.Rows[0][6],
