...
namespace CPC.POS.ViewModel
{
    public partial class PurchaseOrderViewModel : ViewModelBase
    {
        ...
        #region ShowPaymentHistoryCommand

        private ICommand _showPaymentHistoryCommand;
        /// <summary>
        /// Show payment history.
        /// </summary>
        public ICommand ShowPaymentHistoryCommand
        {
            get
            {
                if (_showPaymentHistoryCommand == null)
                {
                    _showPaymentHistoryCommand = new RelayCommand(ShowPaymentHistoryExecute, CanShowPaymentHistoryExecute);
                }
                return _showPaymentHistoryCommand;
            }
        }
        ...
        #region ShowPaymentHistoryExecute

        /// <summary>
        /// Show payment history.
        /// </summary>
        private void ShowPaymentHistoryExecute()
        {
            _dialogService.ShowDialog<POPaymentHistoryView>(_ownerViewModel, new POPaymentHistoryViewModel(_selectedPurchaseOrder.Resource.ToString()), "Payment History");
        }

        #endregion

        #region CanShowPaymentHistoryExecute

        /// <summary>
        /// Determine whether can call ShowPaymentHistoryExecute method.
        /// </summary>
        /// <returns>True will call. Otherwise False.</returns>
        private bool CanShowPaymentHistoryExecute()
        {
            if (_selectedPurchaseOrder == null)
            {
                return false;
        ...
        private void GetProducts()
        {
            try
            {
                lock (UnitOfWork.Locker)
                {
                    ...
                    {
                        ProductCollection = new CollectionBase<base_ProductModel>(productRepository.GetAll(x =>
                            x.ItemTypeId != grouptype && x.ItemTypeId != servicetype && x.IsPurge != true).Select(x => new base_ProductModel(x)));
                    }
                    else
                    {
                        ProductCollection = new CollectionBase<base_ProductModel>(productRepository.GetAll(x =>
                            x.base_ProductStore.Any(y => y.StoreCode == Define.StoreCode) &&
                            x.ItemTypeId != grouptype &&
                            x.ItemTypeId != servicetype && x.IsPurge != true).Select(x => new base_ProductModel(x)));
                    }
                }
            }
            catch (Exception exception)
        ...
        private bool Save()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    decimal totalPaid = resourcePaymentDetailRepository.GetAll(x =>
                        x.base_ResourcePayment.DocumentResource == purchaseOrderId).Sum(x => x.Paid);
                    decimal totalPaidExclude = resourcePaymentDetailRepository.GetAll(x =>
                        x.base_ResourcePayment.DocumentResource == purchaseOrderId &&
                        x.Id != _selectedPurchaseOrder.ResourcePaymentDetail.Id).Sum(x => x.Paid);
                    if (_selectedPurchaseOrder.ResourcePayment.TotalPaid < totalPaid && _selectedPurchaseOrder.ResourcePayment.TotalPaid < totalPaidExclude)
                    {
                        throw new Exception("Total paid is unreasonable. Check payment history.");
                    }
                    ...
                    if (_selectedPurchaseOrder.ResourcePayment.IsNew ||
                        _selectedPurchaseOrder.ResourcePayment.TotalPaid > totalPaid)
                    {
                        // Insert new ResourcePayment.
                        ...
                        // Prepare to insert new ResourcePaymentDetail.
                        if (!_selectedPurchaseOrder.ResourcePaymentDetail.IsNew)
                        {
                            _selectedPurchaseOrder.ResourcePaymentDetail.IsNew = true;
                            _selectedPurchaseOrder.ResourcePaymentDetail.Id = 0;
                            _selectedPurchaseOrder.ResourcePaymentDetail.NewResourcePaymentDetailEntity();
                        }

                        // Calculate PaymentDetail's paid.
                        _selectedPurchaseOrder.ResourcePaymentDetail.Paid = _selectedPurchaseOrder.ResourcePayment.TotalPaid - totalPaid;
                    }
                    else
                    {
                        ...
                        if (_selectedPurchaseOrder.ResourcePayment.IsDirty)
                        {
                            ...
                            // Calculate PaymentDetail's paid.
                            _selectedPurchaseOrder.ResourcePaymentDetail.Paid = _selectedPurchaseOrder.ResourcePayment.TotalPaid - totalPaidExclude;
                        }
                    }
                    ...
                    // Calculate Balance if not payment.
            ...
            try
            {
                lock (UnitOfWork.Locker)
                {
                    ...
                    foreach (base_PurchaseOrderDetail item in purchaseOrderDetails)
                    {
                        ...
                        if (product != null)
                        {
                            purchaseOrderDetailModel.ItemDescription = product.Description;
        ...
        private void ResourcePaymentPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            switch (e.PropertyName)
            {
                case "TotalPaid":
                    ...
                    _selectedPurchaseOrder.IsDirty = true;
                    _selectedPurchaseOrder.ResourcePaymentDetail.IsDirty = true;
