...
namespace CPC.POS.ViewModel
{
    public class LoginViewModel : ViewModelBase, IDataErrorInfo
    {
        ...
        public LoginViewModel(Window loginView)
        {
            ...
            Common.XMLLanguageFileName = string.Format("{0}.xml", this.LanguageItem.CultureInfo.Name);
            Thread.CurrentThread.CurrentUICulture = this.LanguageItem.CultureInfo;
        ...
        {
            return this.ExtensionErrors.Count == 0;
        }
        ...
        private void OnLoginExecuted()
        {
            try
            {
                ...
                if (result.HasValue)
                {
                    if (result.Value)
                    {
                        ...
                        if (this.Remember.HasValue && this.Remember.Value)
                        {
                            ...
                            {
                                MsgControl.ShowWarning("That application saves password is error! /n" + ex.ToString(), "Cảnh báo", MessageBoxControl.MessageBoxButtonCustom.OK);
                            }
                        }
                        else
                        {
                            ...
                            {
                                MsgControl.ShowWarning("That application clears password is error! /n" + ex.ToString(), "Cảnh báo", MessageBoxControl.MessageBoxButtonCustom.OK);
                            }
                        }
                    ...
                    else
                    {
                        ...
                        MsgControl.ShowWarning(this._message, Language.Information, MessageBoxControl.MessageBoxButtonCustom.OK);
                    }
                }
                else
                {   //To count number of logins .
                ...
                {
                    MsgControl.ShowWarning(Application.Current.FindResource("Login_Message_Shutdown") as string, Language.Warning, MessageBoxControl.MessageBoxButtonCustom.OK);
                    this._view.Close();
        ...
        private bool? IsLoginSuccess()
        {
            try
            {
                ...
                base_ResourceAccount resourceAccount = _resourceAccountRepository.GetIQueryable(x => x.LoginName.Trim().ToLower().Equals(this.UserName.Trim().ToLower()) && x.Password.Trim().Equals(encryptPassword)).SingleOrDefault();
                ...
                {
                    if (this.userName.Equals(Define.ADMIN_ACCOUNT)
        ...
        public string this[string columnName]
        {
            get
            {
                ...
                if (this.ExtensionErrors.Count > 0)
