...
namespace CPC.POS.ViewModel
{
    class AttachmentViewModel : ViewModelBase
    {
        ...
        {
            if (MsgControl.ShowWarning(Language.Text4, Language.DeleteItems, MessageBoxButtonCustom.YesNo) == MessageBoxResultCustom.Yes)
            {
                DeleteFolder();
            }
        }
        ...
        {
            if (MsgControl.ShowWarning(Language.Text4, Language.Warning, MessageBoxButtonCustom.YesNo) == MessageBoxResultCustom.Yes)
            {
                DeleteFiles();
            }
        }
        ...
        private void Initialize()
        {
            try
            {
                ...
                string type = FolderIn.Attachment.ToDescription();
                base_VirtualFolder rootFolder = virtualFolderRepository.Get(x => x.ParentFolderId == null && x.IsActived && x.Code == type);
                if (rootFolder == null)
                {
                    // Insert root folder.
        ...
        private void GetFiles()
        {
            try
            {
                lock (UnitOfWork.Locker)
                {
                    ...
                    // Get range of files.
                    IList<base_Attachment> files = attachmentRepository.GetRangeDescending<long>(_selectedFolder.FileCollection.Count, NumberOfDisplayItems, x => x.Id, x => x.VirtualFolderId == _selectedFolder.Id && x.IsActived);
        ...
        private void CreateFolder()
        {
            try
            {
                ...
                base_VirtualFolder folder = new base_VirtualFolder
                {
                    ...
                    Code = FolderIn.Attachment.ToDescription(),
                    DateCreated = now,
                    DateUpdated = now,
                    IsActived = true
                };
        ...
        private void Open(base_AttachmentModel file)
        {
            ...
            catch (Win32Exception win32Exception)
            {
                ...
                {
                    _log4net.Error(string.Format("Message: {0}. Source: {1}", win32Exception.Message, win32Exception.Source));
                    MsgControl.ShowWarning(win32Exception.Message, Language.Warning, MessageBoxButtonCustom.OK);
                }
            }
            catch (Exception exception)
            {
        ...
        private void Scan()
        {
            ...
            {
                MsgControl.ShowWarning(Language.Text17, "Thông báo", MessageBoxButtonCustom.OK);
            }
        }
                    ...
                    else
                    {
                        ...
                        if (string.Compare(currentFolder.FolderName, currentFolder.base_VirtualFolder.FolderName, false) != 0)
                        {
                            ...
                            if (isExist)
                            {
                                // Get old name.
                                currentFolder.FolderName = currentFolder.base_VirtualFolder.FolderName;
                                MsgControl.ShowWarning(string.Format(Language.Text18, currentFolder.FolderName), "Thông báo", MessageBoxButtonCustom.OK);
                            }
                            else
                            {
                                // Update.
                    ...
                    else
                    {
                        ...
                        if (string.Compare(currentAttachment.FileName, currentAttachment.base_Attachment.FileName, false) != 0)
                        {
                            ...
                            if (isExist)
                            {
                                // Get old name.
                                currentAttachment.FileName = currentAttachment.base_Attachment.FileName;
                                MsgControl.ShowWarning(string.Format(Language.Text19, currentAttachment.FileName), "Thông báo", MessageBoxButtonCustom.OK);
                            }
                            else
                            {
                                // Update.
