...
namespace CPC.POS.ViewModel
{
    class SalesOrderPaymenViewModel : ViewModelBase
    {
        ...
        private bool OnAcceptedPaymentCommandCanExecute()
        {
            ...
            {
                decimal totalPaid = SaleOrderModel.PaymentCollection.Sum(x => x.TotalPaid - x.Change);
                return PaymentModel.IsDirty && ((PaymentModel.TotalAmount <= 0 && _balance < totalPaid) || (PaymentModel.TotalAmount > 0 && PaymentModel.TotalPaid > 0)) && ((IsPaidFull && PaymentModel.Balance == 0) || !IsPaidFull);
            }
        }
            ...
            if (Define.CONFIGURATION.AcceptedPaymentMethod.HasValue)
            {
                foreach (ComboItem paymentMethod in Common.PaymentMethods.Where(x => !x.Islocked))
                {
                    ...
                    if (Define.CONFIGURATION.AcceptedPaymentMethod.Value.Has((int)paymentMethod.Value))
                    {
                        ...
                        paymentDetailModel.IsCreditCard = false;//Show coloumn Tip when creditcard
                        paymentDetailModel.IsPaymentCardMethod = false;
