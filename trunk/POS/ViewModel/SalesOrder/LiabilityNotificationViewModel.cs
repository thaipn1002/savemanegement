...
{
    public class LiabilityNotificationViewModel : OrderViewModel
    {
        #region Define
        ...
        public LiabilityNotificationViewModel()
            : base()
        {
        ...
        public LiabilityNotificationViewModel(bool isList, object param)
            : this()
        {
        ...
        private void OnDoubleClickViewCommandExecute(object param)
        {
            ...
            Window window = Application.Current.Windows.OfType<Window>().FirstOrDefault(x => x.IsActive == true);
            if (window != null)
            {
        ...
        private void LoadDataByPredicate(Expression<Func<base_SaleOrder, bool>> predicate, bool refreshData = false, int currentIndex = 0)
        {
            ...
            bgWorker.DoWork += (sender, e) =>
            {
                ...
                lock (UnitOfWork.Locker)
                {
                    ...
                    //Get data with range
                    IOrderedEnumerable<base_SaleOrder> saleOrders = _saleOrderRepository.GetAll(predicate).OrderByDescending(x => x.DueDate);//(currentIndex, TotalSaleOrder, x => x.OrderDate.Value, predicate);
        ...
        private Expression<Func<base_SaleOrder, bool>> CreatePredicateWithConditionSearch(string keyword)
        {
            ...
            // ngay bao= ngay tra - ngay hien tai
            predicate = predicate.And(x => (x.DueDate.HasValue && x.DueDate <= DateTime.Now) || ((x.DueDate.HasValue && x.DueDate <= dateTime) && (x.DueDate.HasValue && x.DueDate > DateTime.Now)));
