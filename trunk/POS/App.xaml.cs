...
namespace CPC.POS
{
    ...
    public partial class App : Application, ISingleInstanceApp
    {
        ...
        private void InitialData()
        {
            ...
            {
                m_Logger.Error(ex);
                MessageBox.Show(ex.ToString(), "Configuration");
                throw;
            }
        }
        ...
        #region SetTrialVersion
        private bool SetTrialVersion()
        {
            //To add new date of trial version.
            if (CPC.POS.Properties.Settings.Default.TrialDate.Year == 1)
            {
                CPC.POS.Properties.Settings.Default.TrialDate = DateTimeExt.Now.AddDays(CPC.POS.Properties.Settings.Default.TrialDays);
                CPC.POS.Properties.Settings.Default.Save();
                return true;
            }
            //To add check date of trial version if it's expired , application will shutdown.
            else
            {
                if (DateTimeExt.Now.Date.Month >= CPC.POS.Properties.Settings.Default.TrialDate.Month
                    && DateTimeExt.Now.Date.Day >= CPC.POS.Properties.Settings.Default.TrialDate.Day
                    && DateTimeExt.Now.Date.Year >= CPC.POS.Properties.Settings.Default.TrialDate.Year)
                {
                    MsgControl.ShowWarning("Phần mềm của bạn đã hết hạn sử dụng.Vui lòng liên hệ nhà cung cấp để cập nhật lại phiên bản mới.", "Thông báo", MessageBoxControl.MessageBoxButtonCustom.OK);
                    return false;
                }
                return true;
            }
        ...
        protected override void OnStartup(StartupEventArgs e)
        {
            ...
            {
                base.OnStartup(e);
                CPC.POS.Properties.Settings.Default.TrialDate = DateTime.Now.AddDays(15);
                CPC.POS.Properties.Settings.Default.Save();
                bool resultTrial = this.SetTrialVersion();
                if (resultTrial)
                {
                    // Configure Log4Net
